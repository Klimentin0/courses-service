...
func gentoken() error {
	...
	file, err := os.Open("config/keys/54bb2165-71e1-41a6-af3e-7da4a0e1e2c1.pem")
	if err != nil {
		return fmt.Errorf("opening key file: %w", err)
	}
	defer file.Close()

	// limit PEM file size to 1megabyte. Thie should be reasonable for
	// almost any PEM file and prevents shenanigans like linking the file
	// to /dev/random or smt like that.
	...
	if err != nil {
		return fmt.Errorf("parsing auth privae key: %w", err)
	}

	// Generating a token requires defining a set of claims. In this applications
	// case, we only care about defining the subject and the user in question and
	// the roles they have on the database. This token will wxpire in a year.
	//
	...
	// jti (JW T ID): Unique identifier; can be used to prevent the JWT from being replayed(allows a token to be used only once).
	claims := struct {
		...
		RegisteredClaims: jwt.RegisteredClaims{
			Subject:   "12345678789",
			Issuer:    "courses-service project",
...
func opaPolicyEvaluation(ctx context.Context, opaPolicy string, input any) error {
	const opaPackage string = "klim.rego"
...
func genkey() (*rsa.PrivateKey, error) {
	...
	// Contruct a PEM block for the private key.
